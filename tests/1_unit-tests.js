...
suite('UnitTests', () => {
  ...
  suite('Function validSudokuInput()', () => {
    test('Valid "1-9" characters', (done) => {
      const input = ['1', '2', '3', '4', '5', '6', '7', '8', '9'];
      input.forEach((number) => {
        assert.isTrue(Solver.validSudokuInput(number));
      });
      done();
    });
    ...
    test('Invalid characters (anything other than "1-9") are not accepted', (done) => {
      ...
      input.forEach((invalidValue) => {
        assert.isFalse(Solver.validSudokuInput(invalidValue));
      });
      done();
    });
  });

  suite('Function parseSudokuString()', () => {
    test('Parses a valid puzzle string into an object', (done) => {
      ...
      assert.deepStrictEqual(Solver.parseSudokuString(input), expected);
      done();
    });
    ...
    test('Shows an error for puzzles that are not 81 numbers long', (done) => {
      ...
      Solver.parseSudokuString(shortStr);
      assert.strictEqual(errorDiv.innerText, errorMsg);

      Solver.parseSudokuString(longStr);
      assert.strictEqual(errorDiv.innerText, errorMsg);
      done();
    });
  });
  ...
  suite('Function validateGrid()', () => {
    ...
    test('Valid puzzles pass', (done) => {
      ...
      const grid = Solver.parseSudokuString(input);
      assert.isTrue(Solver.validateGrid(grid));
      done();
    });
    ...
    test('Invalid puzzles fail', (done) => {
      ...
      const grid = Solver.parseSudokuString(input);
      assert.isFalse(Solver.validateGrid(grid));
      done();
    });
  });
  ...
  suite('Function solveSudoku()', () => {
    ...
    test('Returns the expected solution for an incomplete puzzle', (done) => {
      ...
      assert.deepStrictEqual(Solver.solveSudoku(input), expected);
      done();
    });
  });
});
